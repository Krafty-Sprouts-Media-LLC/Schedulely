...
(function($) {
    ...
    function initAuthorSelect() {
        const $select = $('#schedulely_excluded_authors');
        ...
        if (typeof $select.select2 === 'function') {
            $select.select2({
                placeholder: 'Select authors to exclude',
                allowClear: true,
                width: '100%'
            });
        }
    ...
    function displayCapacityResult(capacityData) {
        ...
        if (!capacityData.valid) {
            $notice.html(
                ...
                (capacityData.error || 'Your time settings are invalid.') +
                '</div>'
            );
            $suggestions.hide();
            return;
        ...
        if (capacityData.meets_quota) {
            $notice.html(
                ...
                '<strong>✅ Your settings can fit approximately ' + capacityData.capacity + ' posts per day</strong><br>' +
                '<small>Time window: ' + capacityData.start_time + ' - ' + capacityData.end_time + ' | ' +
                'Interval: ' + capacityData.min_interval + ' minutes | ' +
                'Estimate accounts for random time placement</small>' +
                '</div>'
            );
            $suggestions.hide();
        } else {
            $notice.html(
                ...
                '<strong>⚠️ Your settings can only fit approximately ' + capacityData.capacity + ' posts, but you want ' + capacityData.desired_quota + ' posts</strong><br>' +
                '<small>With random time scheduling, fewer posts will fit. Adjust settings below.</small>' +
                '</div>'
            );
            ...
            if (capacityData.suggestions && capacityData.suggestions.length > 0) {
                ...
                capacityData.suggestions.forEach(function(suggestion, index) {
                    ...
                    if (suggestion.type === 'reduce_interval') {
                        actionButton = '<button type="button" class="button button-small schedulely-apply-suggestion" ' +
                            'data-type="interval" data-value="' + suggestion.suggested + '">Apply</button>';
                    } else if (suggestion.type === 'reduce_quota') {
                        actionButton = '<button type="button" class="button button-small schedulely-apply-suggestion" ' +
                            'data-type="quota" data-value="' + suggestion.suggested + '">Apply</button>';
                    } else if (suggestion.type === 'expand_window') {
                        actionButton = '<button type="button" class="button button-small schedulely-apply-suggestion" ' +
                            'data-type="endtime" data-value="' + suggestion.suggested_end + '">Apply</button>';
                    }
                    
                    suggestionsHtml += 
                        '<div class="schedulely-suggestion-item">' +
                        '<div class="schedulely-suggestion-content">' +
                        '<strong>' + (index + 1) + '. ' + suggestion.label + '</strong><br>' +
                        '<span>' + suggestion.message + '</span>' +
                        '</div>' +
                        '<div class="schedulely-suggestion-action">' +
                        actionButton +
                        '</div>' +
                        '</div>';
                });
