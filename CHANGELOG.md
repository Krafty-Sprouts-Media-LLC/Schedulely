...
- **`max_tokens` raised to 32,768 in timezone mode** — a 1,500-post timezone response (`ordered_ids` + `timezone_groups`) requires ~23,000 output tokens. The previous default of 16,384 would truncate the response for pools over ~700 posts. Standard (non-timezone) mode keeps the 16,384 default. DeepSeek V4-flash/pro support this output size; the 1M token context window means input size is never the constraint.
