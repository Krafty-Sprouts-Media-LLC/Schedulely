...
### Fixed
- **Restored JSON output mode on the WP-AI reorder path — the ingredient lost when moving from the pre-connector legacy path to the WordPress AI client.** Investigating two 300-post runs (one failing with `schedulely_ai_shape`, one "succeeding" only after reconciliation patched a 301-ID, duplicate-laden array) traced the regression to a concrete cause: the old direct-HTTP path (which ordered ~700 posts in 1.5.x) sent `response_format` to DeepSeek, but the WP AI client path never did. Without it the model free-forms plain text, loops emitting duplicate IDs, and runs into its own 64K output ceiling before ever closing the JSON array — producing unparseable output. The builder chain now calls `->as_json_response()`, which the OpenAI-compatible provider (DeepSeek) translates to `response_format: {"type":"json_object"}`, constraining the decoder to emit parseable, properly-closed JSON. The prompt already contains the word "JSON", satisfying DeepSeek's json_object precondition, and the system instruction now also includes a concrete worked example (4 inputs → a complete `{"ordered_ids":[...]}` response) — DeepSeek's documented mitigation for json_object mode "occasionally returning empty content." This is not a magic loop-cure — reconciliation still backstops a wrong multiset — but it restores the guardrail that made the AI path survivable at scale. (Note: the diagnosis also showed the 1.7.14 `temperature 0.3` change did not help and the working 1.5.x path used `temperature 1.0`; the real lever was always the output-format constraint, not creativity.)

- **Set a deliberate 64K (`65536`) `max_tokens` on the reorder request as a runaway-loop fuse.** The Try-1 failure stopped at exactly `65,536` output tokens — a power of two that turned out to be an *accidental* default applied by the client stack, not the model's limit: `deepseek-v4-flash`'s true maximum output is **384K**. So the AI path was never truly "uncapped" — leaving `max_tokens` unset would let a degenerate repetition loop run toward 384K (many minutes, large cost) before failing. This is **not** the kind of low cap that truncates a valid answer (a legitimate 1,500-post reorder is only ~12–15K tokens; 64K is ~4× that); it is a ceiling set well above any real response and well below the runaway. DeepSeek's JSON-mode guide also explicitly recommends setting `max_tokens` to prevent the JSON being truncated mid-string. Applied on both the WP-AI path (`->using_max_tokens()`) and the legacy HTTP body, filterable via `schedulely_ai_reorder_max_tokens`, floored at 1 and capped at the model's 384K documented maximum.
...
### Fixed
- **Lowered the AI reorder temperature and tightened the prompt to curb the token-loop failure mode.** A "successful" 300-post reorder revealed the real problem: the model emitted **528 IDs for 300 posts** (~45 unique repeated) and burned **29,344 output tokens** (~189s), with reconciliation salvaging only ~15% as genuine ordering. The reorder is a permutation, not a creative task, yet we were sending it at `temperature 1.0`, which invites degenerate repetition. Two changes target the loop without limiting legitimate large responses: (1) `using_temperature()` now defaults to **0.3** (filter `schedulely_ai_reorder_temperature`) instead of 1.0, so the model produces a clean finite answer rather than wandering; (2) the user prompt and system instruction now state the exact stop condition — "all N IDs, each exactly once, array length exactly N, stop after the N-th ID; never repeat an ID already emitted." Output tokens are deliberately **not** capped: a correct answer for a 1,500-post pool is itself a long array, so a ceiling would truncate valid responses into broken JSON. The legacy HTTP path uses the same temperature helper for consistency.

### Changed
- **Raised the AI reorder request timeout to an effectively-uncapped finite value (3600s); do NOT use 0.** Every prior *small* timeout was a guess that cut off a request the model was still working on (the `cURL error 28 ... after 195002 ms` failures). The fix is a generous budget, not a magic sentinel: an interim attempt at `0` ("wait indefinitely") backfired because the WP AI client routes through WordPress's own HTTP client, where `0` collapses to a **~1-second** timeout and fails instantly (`cURL error 28 ... after 1003 ms`). So `get_wp_ai_timeout_filter()` now defaults to **3600s** (filter `schedulely_ai_reorder_timeout`) and the legacy HTTP path's `schedulely_ai_request_timeout` likewise defaults to **3600s**; the old 1200s legacy cap and `schedulely_ai_request_timeout_max` clamp are gone. 3600s is far beyond what the model needs, so it never cuts off a working request, while avoiding the `0` pitfall. NOTE: this only runs as long as PHP `max_execution_time` and the web/cron server's request timeout allow — if a run dies at a round number (e.g. 300s), that ceiling is in PHP-FPM/nginx, not here.
- **Steered the AI reorder away from DeepSeek's "thinking" model and added a non-thinking fallback.** Per DeepSeek's docs, the current models are `deepseek-v4-flash` / `deepseek-v4-pro`; the legacy IDs auto-reroute — `deepseek-chat` to v4-flash in **non-thinking** mode, `deepseek-reasoner` to v4-flash in **thinking** mode. The installed DeepSeek connector still exposes only the legacy IDs, so a reorder running through it could land on `deepseek-reasoner`, whose internal reasoning stream is the likely source of the multi-minute, ~29k-token runs (the model "thinks" at length on a task that needs no reasoning). The `wpai_preferred_text_models` preference list now includes `deepseek-chat` as an explicit fallback after the v4 IDs — guaranteeing the fast, non-reasoning path even on connectors that only know the legacy names — and `deepseek-reasoner` is deliberately excluded. This is the model the feature used successfully before the timezone work.
