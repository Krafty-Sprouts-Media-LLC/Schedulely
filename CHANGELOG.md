...
### Changed
- **Removed the AI reorder request timeout ceiling — the request now runs uncapped.** Every prior timeout value we set was a *guess* that ended up cutting off a request the model was still working on (the `cURL error 28 ... after 195002 ms` failures). A large reorder can legitimately take many minutes, so we no longer impose a ceiling. `get_wp_ai_timeout_filter()` now defaults to **0** (filter `schedulely_ai_reorder_timeout`), which flows through the WP AI client → php-ai-client → Guzzle, where `timeout = 0` means "wait indefinitely"; the legacy HTTP path's `schedulely_ai_request_timeout` default is likewise **0** (cURL `0` = no timeout), and its old 1200s cap and `schedulely_ai_request_timeout_max` clamp are gone. Only negative overrides are guarded. NOTE: an uncapped cURL/Guzzle call still only runs as long as PHP `max_execution_time` and the web/cron server's own request timeout allow — if a run dies at a round number (e.g. 300s) with the client uncapped, that ceiling is in PHP-FPM/nginx, not here.
